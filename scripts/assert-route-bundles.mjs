...
//
// FORCING EXAMPLES. (a) 2026-07-12, prod: a sitewide scan found 172 routes in
// JSP_BY_ROUTE with no JSP wrapper on disk (a news article, the neon-surge-loop
// game + its 6-locale guides, black-hole/galaxy locale guides). Nothing could
// deploy to prod - not even an unrelated breadcrumb fix - until the whole
// backlog was resynced from staging. (b) 2026-07-28, staging: the
// guide-farm-prune lane committed scripts/site-data.mjs wholesale while a
// concurrent new-tool-discovery lane (fire395) had 15 half-finished
...
//
// The G54-JSP pre-push gate already covers this, but it lives in the WRAPPER
// tree (.agent/skills/_lib/route-bundle-completeness.mjs), outside this repo, so
// it only protects machines where install-prod-mirror-hooks.sh has been run.
// With lanes running on more than one machine that is not a guarantee. This
// script is self-contained IN the repo so CI can run it on every push, and it
// fails in seconds listing EVERY gap instead of one-at-a-time after a long
// render. It complements (never replaces) the pre-push gate.
//
